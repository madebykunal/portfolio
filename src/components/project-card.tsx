...
export function ProjectCard({ project, eager = false }: { project: Project; eager?: boolean }) {
  return (
    <a
      ...
      className="group flex h-full flex-col bg-card p-2 no-underline shadow-card transition duration-200 ease-out hover:-translate-y-0.5 hover:shadow-lift motion-reduce:transition-none motion-reduce:hover:translate-y-0"
    >
      <div className={`overflow-hidden rounded-xs px-5 pt-5 ${project.tint}`}>
        <Image
          src={project.shot}
          alt={project.shotAlt}
          sizes={SHOT_SIZES}
          ...
          className="aspect-[5/3] w-full rounded-t-xs object-cover object-top shadow-shot"
        />
      </div>

      <div className="px-1 pt-3.5 pb-2">
        <div className="flex items-center gap-x-2">
          <Image src={project.logo} alt="" width={16} height={16} className="rounded-xs" />
